DROP VIEW IF EXISTS tabfm_warmup_runners;

CREATE VIEW tabfm_warmup_runners AS
WITH eligible_races AS (
    ...
    HAVING COUNT(*) >= 4
       ...
       AND SUM(CASE WHEN is_trainable = 1 THEN 0 ELSE 1 END) = 0
...
JOIN eligible_races AS eligible USING (race_id);
