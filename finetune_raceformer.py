...
from src.raceformer_partition import combine_disjoint_snapshots, partition_by_validation_ids
from train_raceformer import _batches, _pad_batch, _selected_indices, _selection, evaluate
...
def main() -> None:
    ...
    saved_validation_ids = partition.get("validation_race_ids")
    if saved_validation_ids is not None:
        ...
        (
            ...
            np.asarray(saved_validation_ids, dtype=np.int64),
        )
        print(
            f"reused_checkpoint_partition validation_races={len(saved_validation_ids)}",
            flush=True,
        )
    ...
    effective_hyperparameters = {
        ...
        "data": {
            ...
            "partition_mode": source.get("partition", {}).get(
                "mode", "legacy_database_views"
            ),
        },
        "preprocessing": {
